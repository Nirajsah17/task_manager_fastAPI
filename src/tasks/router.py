...
def update_one_task(body: TaskSchema, task_id: int, db: Session = Depends(get_db), user: UserModel = Depends(is_authenticated)):
...
def delete_one_task(task_id, db: Session = Depends(get_db), user: UserModel = Depends(is_authenticated)):
